...
또는 Finder에서 `CC Manager.command` 더블클릭. `http://localhost:8765` 가 자동으로 열린다.

개발 중엔 `CC Manager (Dev).command` — 파일 저장할 때마다 문법 체크 후 자동 재시작.
...
**라이브러리 정리**
- 안전한 삭제: `~/Library/Application Support/Sims4CCManager/trash/` 로 이동 (복원 가능)
...
```
~/Library/Application Support/Sims4CCManager/
